...
print_stackframe(void) {
     ...
     /* get current value of %ebp and %eip */
     ebp = read_ebp();
     eip = read_eip();
     for (i = 0; ebp !=0 && i < STACKFRAME_DEPTH; i++) {
         cprintf("ebp: 0x%08x eip: 0x%08x args: 0x%08x 0x%08x 0x%08x 0x%08x", ebp, eip, *((uint32_t *)ebp + 2), *((uint32_t *)ebp + 3), *((uint32_t *)ebp + 4), *((uint32_t *)ebp + 5));
         cprintf("\n");
         print_debuginfo(eip - 1); // why minus one ?
         eip = *((uint32_t *)ebp + 1);
         ebp = *(uint32_t *)ebp;
     }
}
