...
#define BEING_CHECK_VALID_VADDR 0X1000 // Page size: 4K
...
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)
{
      ...
      */
     cprintf("SWAP: load ptep 0x%08x swap entry %d to vaddr 0x%08x, page 0x%08x, No %d\n", ptep, (*ptep)>>8, addr, result, (result-pages));
...
check_swap(void)
{
    //backup mem env
     ...
     for (i= 0;i<CHECK_VALID_PHY_PAGE_NUM;i++) {
         ...
         //cprintf("i %d, check_ptep addr 0x%08x, value %x\n", i, check_ptep[i], *check_ptep[i]);
